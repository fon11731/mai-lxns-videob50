...
# def test_youtube_downloader(downloader, title_name, difficlty_name):
#     # 检查是否已有搜索结构缓存
#     if not os.path.exists(f"./videos/search_result.json"):
#         print("未找到搜索结果缓存，正在搜索视频...")
#         # 搜索视频
#         keyword = f"{title_name} {difficlty_name} AP【maimaiでらっくす外部出力】"
#         videos = downloader.search_video(keyword)
#         # 保存搜索结果，以便缓存使用
#         with open(f"./videos/search_result.json", "w") as f:
...
#     # 从缓存中读取搜索结果
#     with open(f"./videos/search_result.json", "r") as f:
#         videos = json.load(f)
#     # 下载videos列表中的视频
#     for video in videos:
#         output_name = f"{title_name}_{difficlty_name}"
#         downloader.download_video(video['url'], output_name, output_path, high_res=True)

# if __name__ == "__main__":
    
#     proxy = "127.0.0.1:7890"  # 使用本地代理地址
#     # api_key = "AIzaSyDc3_gvbRXDz_xh6YowZXOAxiy30hhh0hg"
#     # downloader = YouTubeDownloader(api_key, proxy)
#     downloader = PurePytubefixDownloader(proxy)
#     output_path = "./videos/downloads"

#     title_name = "GEOMETRIC DANCE"
#     difficlty_name = "Master"
#     test_youtube_downloader(downloader, title_name, difficlty_name)
